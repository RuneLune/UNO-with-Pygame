...
class BotBButton(GameObject, metaclass=SingletonMeta):
    @overrides
    def start(self) -> None:
        self.bot_b_text = BotBText()
        self.image = pygame.image.load(image_resource(join("lobby", "bot_B.png")))
        self.rect = self.image.get_rect()
        self.name = "BotB_Button"
    ...
    def on_mouse_enter(self) -> None:
        ...
        self.bot_b_text.visible()
        self.bot_b_text.enable()
        return None

    @overrides
    def on_mouse_exit(self) -> None:
        self.image = self.img_copy
        self.bot_b_text.invisible()
        return None

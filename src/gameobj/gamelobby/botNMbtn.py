...
class BotNormalButton(GameObject, metaclass=SingletonMeta):
    @overrides
    def start(self) -> None:
        self.bot_normal_text = BotNormalText()
        self.image = pygame.image.load(image_resource(join("lobby", "bot_Normal.png")))
        self.rect = self.image.get_rect()
        self.name = "BotNormal_Button"
    ...
    def on_mouse_enter(self) -> None:
        ...
        self.bot_normal_text.visible()
        self.bot_normal_text.enable()
        return None

    @overrides
    def on_mouse_exit(self) -> None:
        self.image = self.img_copy
        self.bot_normal_text.invisible()
        return None

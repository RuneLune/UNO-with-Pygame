...
class BotDButton(GameObject, metaclass=SingletonMeta):
    @overrides
    def start(self) -> None:
        self.bot_d_text = BotDText()
        self.image = pygame.image.load(image_resource(join("lobby", "bot_D.png")))
        self.rect = self.image.get_rect()
        self.name = "BotD_Button"
    ...
    def on_mouse_enter(self) -> None:
        ...
        self.bot_d_text.visible()
        self.bot_d_text.enable()
        return None

    @overrides
    def on_mouse_exit(self) -> None:
        self.image = self.img_copy
        self.bot_d_text.invisible()
        return None

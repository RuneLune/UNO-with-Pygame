...
class BotCButton(GameObject, metaclass=SingletonMeta):
    @overrides
    def start(self) -> None:
        self.bot_c_text = BotCText()
        self.image = pygame.image.load(image_resource(join("lobby", "bot_C.png")))
        self.rect = self.image.get_rect()
        self.name = "BotC_Button"
    ...
    def on_mouse_enter(self) -> None:
        ...
        self.bot_c_text.visible()
        self.bot_c_text.enable()
        return None

    @overrides
    def on_mouse_exit(self) -> None:
        self.image = self.img_copy
        self.bot_c_text.invisible()
        return None
